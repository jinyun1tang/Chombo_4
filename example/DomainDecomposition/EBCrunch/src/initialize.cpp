#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdbool.h>
#include <math.h>
#include <ctype.h>

#include "crunchflow.h"

#ifdef __cplusplus
extern"C" {
#endif

  char *trimwhitespace(char *str);
  int maxval(int m, int n, int a[][n]);
  void printarray(const char *name, int m, int n, int l, double a[][n][l]);
  void printintarray(const char *name, int m, int n, int l, int a[][n][l]);

  void dumpMuMin(const int np, 
		 const int nkin, 
		 const int ncomp, 
		 const int nspec,
		 const double mumin[ncomp][nkin][MAX_PATH],
		 char *ulab[ncomp + nspec]);

  // Initialized in parseInput()
  //
  int ncomp;
  int nspec;
  int nkin;
  int ntemp = 0;
  bool read_kinetics = false;

  // Linkage with crunchflowloop()
  //
  bool RunIsothermal = false; 
  int (* ndepend)[MAX_PATH];
  int (* nreactmin);
  int (* idepend)[MAX_PATH][MAX_DEPEND];
  int (* itot_min)[MAX_PATH][MAX_DEPEND];
  void *ulabGlobal;
  double bvec[NBASIS]; 
  double vec[NTEMP][NBASIS];
  double vecgam[NTEMP][NBASIS];
  void *as1Global;
  void *eqhomGlobal;
  void *alnkGlobal;
  double tempc[TPOINTS];
  double a_dh[TPOINTS];
  double b_dh[TPOINTS];
  double b_dot[TPOINTS];
  double adh[TPOINTS];
  double bdh[TPOINTS];
  double bdot[TPOINTS];
  void *chgGlobal;
  void *acmpGlobal;
  double adhcoeff[NBASIS];
  double bdhcoeff[NBASIS];
  double bdtcoeff[NBASIS];
  void *muaqGlobal;
  void *muminGlobal;
  double (* depend)[MAX_PATH][MAX_DEPEND];
  double (* rate0)[MAX_PATH];
  double (* AffinityDepend1)[MAX_PATH];
  double (* volmol);
  void *spGlobal;
  void *sp10Global;
  void *spoldGlobal;
  void *gamGlobal;
  void *sGlobal;
  void *snGlobal;
  void *sionGlobal;
  void *tGlobal;

  int initialize(const int ncompchombo, 
		 const int nspecchombo, 
		 const int nkinchombo,
		 double inflowchombo[ncompchombo],
		 double inflowchombosp10[ncompchombo],
		 double initchombo[ncompchombo],
		 double initchombosp[ncompchombo + nspecchombo],
		 double initchombosp10[ncompchombo + nspecchombo],
		 double stoichchombo[ncompchombo][nkinchombo][1], 
		 const int nx, 
		 const int ny, 
		 const int nz)
  {
    double wtaq[ncompchombo + nspecchombo];
    char *namcx[nspecchombo];
    char *namin[nkinchombo];

    nreactmin = (int (*))calloc(nkinchombo, sizeof(int));
    volmol = (double (*))calloc(nkinchombo, sizeof(double));

    ndepend = (int (*)[MAX_PATH])calloc(nkinchombo * MAX_PATH, sizeof(int));
    AffinityDepend1 = (double (*)[MAX_PATH])calloc(nkinchombo * MAX_PATH, sizeof(double));
    rate0 = (double (*)[MAX_PATH])calloc(nkinchombo * MAX_PATH, sizeof(double));

    idepend = (int (*)[MAX_PATH][MAX_DEPEND])calloc(nkinchombo * MAX_PATH * MAX_DEPEND, sizeof(int));
    itot_min = (int (*)[MAX_PATH][MAX_DEPEND])calloc(nkinchombo * MAX_PATH * MAX_DEPEND, sizeof(int));
    depend = (double (*)[MAX_PATH][MAX_DEPEND])calloc(nkinchombo * MAX_PATH * MAX_DEPEND, sizeof(double));

    // ncomp+nspec,nx,ny,nz
    //
    // double sp[1][32][32][15]; // allocate(sp(ncomp+nspec,nx,ny,nz),stat=ierr); sp = 0.0d0
    double (* sp)[ny][nx][ncompchombo + nspecchombo];
    sp = (double (*)[ny][nx][ncompchombo + nspecchombo])calloc((ncompchombo + nspecchombo) * nx * ny * nz, sizeof(double));
    spGlobal = sp;

    // double sp10[1][32][32][15]; // allocate(sp10(ncomp+nspec,nx,ny,nz),stat=ierr); sp10 = 0.0d0
    double (* sp10)[ny][nx][ncompchombo + nspecchombo];
    sp10 = (double (*)[ny][nx][ncompchombo + nspecchombo])calloc((ncompchombo + nspecchombo) * nx * ny * nz, sizeof(double));
    sp10Global = sp10;

    // double spold[1][32][32][15]; // allocate(spold(ncomp+nspec,nx,ny,nz),stat=ierr); spold = 0.0d0
    double (* spold)[ny][nx][ncompchombo + nspecchombo];
    spold = (double (*)[ny][nx][ncompchombo + nspecchombo])calloc((ncompchombo + nspecchombo) * nx * ny * nz, sizeof(double));
    spoldGlobal = spold;

    // double gam[1][32][32][15]; // allocate(gam(ncomp+nspec,nx,ny,nz),stat=ierr); gam = 0.0d0
    double (* gam)[ny][nx][ncompchombo + nspecchombo];
    gam = (double (*)[ny][nx][ncompchombo + nspecchombo])calloc((ncompchombo + nspecchombo) * nx * ny * nz, sizeof(double));
    gamGlobal = gam;

    // double s[1][32][32][6]; // allocate(s(neqn,nx,ny,nz),stat=ierr); s = 0.0d0
    double (* s)[ny][nx][ncompchombo];
    s = (double (*)[ny][nx][ncompchombo])calloc(ncompchombo * nx * ny * nz, sizeof(double));
    sGlobal = s;

    // double sn[1][32][32][6]; // allocate(sn(neqn,nx,ny,nz),stat=ierr); s = 0.0d0
    double (* sn)[ny][nx][ncompchombo];
    sn = (double (*)[ny][nx][ncompchombo])calloc(ncompchombo * nx * ny * nz, sizeof(double));
    snGlobal = sn;

    // double sion[1][32][32]; // allocate(sion(nx,ny,nz),stat=ierr); sion = 0.0d0
    double (* sion)[ny][nx];
    sion = (double (*)[ny][nx])calloc(nx * ny * nz, sizeof(double));
    sionGlobal = sion;

    // double t[1][32][32]; // allocate(t(nx,ny,nz),stat=ierr); t = 25.0d0
    double (* t)[ny][nx];
    t = (double (*)[ny][nx])calloc(nx * ny * nz, sizeof(double));
    tGlobal = t;

    double (* mumin)[nkinchombo][MAX_PATH];
    mumin = (double (*)[nkinchombo][MAX_PATH])calloc(ncompchombo * nkinchombo * MAX_PATH, sizeof(double));
    muminGlobal = mumin;

    double (* muaq)[nspecchombo];
    muaq = (double (*)[nspecchombo])calloc(ncompchombo * nspecchombo, sizeof(double));
    muaqGlobal = muaq;

    double (* acmp);
    acmp = (double (*))calloc(ncompchombo * nspecchombo, sizeof(double));
    acmpGlobal = acmp;

    double (* chg);
    chg = (double (*))calloc(ncompchombo * nspecchombo, sizeof(double));
    chgGlobal = chg;

    double (* alnk);
    alnk = (double (*))calloc(nkinchombo * MAX_PATH, sizeof(double));
    alnkGlobal = alnk;

    double (* eqhom);
    eqhom = (double (*))calloc(nspecchombo, sizeof(double));
    eqhomGlobal = eqhom;

    char *(* ulab);
    ulab = (char * (*))calloc(ncompchombo + nspecchombo, sizeof(char *));
    ulabGlobal = ulab;

    // [NBASIS][nspec + nkin * max_path]
    double (* as1)[nspecchombo + nkinchombo * MAX_PATH];
    as1 = (double (*)[nspecchombo + nkinchombo * MAX_PATH])calloc(NBASIS * (nspecchombo + nkinchombo * MAX_PATH), sizeof(double));
    as1Global = as1;

    // dppt = (double (*)[ny][nx][nkinchombo])calloc(nz * ny * nx * nkinchombo, sizeof(double));
    // u_rate = (double (*)[ny][nx][nkinchombo])calloc(nz * ny * nx * nkinchombo, sizeof(double));
    // area = (double (*)[ny][nx][nkinchombo])calloc(nz * ny * nx * nkinchombo, sizeof(double));
    // volfx = (double (*)[ny][nx][nkinchombo])calloc(nz * ny * nx * nkinchombo, sizeof(double));

    // local variables
    //
    int neqn;
    int iflgint = 0;
    int INT[NTEMP];
    int ntt;
    int nncnt;
    double temptmp[NTEMP];
    int nbasis = NBASIS;
    double temp = 25.0; // C
    int ndim2, ndim3;
    double initgamma[ncompchombo + nspecchombo];
    int np = 0;
    int nd = 0;
    int ndim1;

    read_kinetics = false; // global

    //printf("Invoking initialize_cpp...");
    if(DEBUG)
      fprintf(stderr,"Invoking initialize_cpp (ncomp=%d,nspec=%d,nkin=%d,nx=%d,ny=%d,nz=%d)...\n", 
	      ncompchombo,
	      nspecchombo,
	      nkinchombo,
	      nx,
	      ny,
	      nz);

    //printf("Invoking initialize_cpp (ncomp=%p)...\n", ncomp);

    //      REAL(DP), DIMENSION(nbasis), INTENT(IN OUT)                :: bvec
    //      ALLOCATE(bvec(5))
    // bvec = (double *) calloc(NBASIS + 1, sizeof(double)); // + 1 to start indexing at 1

    //      REAL(DP), DIMENSION(:,:), allocatable                :: vec
    //      ALLOCATE(vec(5,8))
    //vec = (double *) calloc(NBASIS * NTEMP, sizeof(double));

    for(size_t i = 0; i < NTEMP; ++i)
      for(size_t j = 0; j < NBASIS; ++j)
	{
	  vec[i][j] = 0.0; // clear vec
	  vecgam[i][j] = 0.0; // clear vecgam
	}
    
    // for(size_t k = 0; k < nz; ++k)
    //   for(size_t j = 0; j < ny; ++j)
    // 	for(size_t i = 0; i < nx; ++i)
    // 	  {
    // 	    H2Oreacted[k][j][i] = 1.0;
    // 	  }

    //    temp = 25.0;
    RunIsothermal = false; 
	
    //     ________________________ DEBYE HUCKEL _____________________
    //
    for(int i = 0; i < TPOINTS; i++)
      {
    	tempc[i] = -333.0;
    	a_dh[i] = -333.0;
    	b_dh[i] = -333.0;
    	b_dot[i] = -333.0;
      }

    //       open(unit=112,file='input_test.txt',status='unknown')
    char* filename = "input_test.txt";
    if(DEBUG)
      printf("parsing %s...\n",filename);
    int status = parseInput(filename);
    if(DEBUG)
      printf("  done\n");

    // temp = 25.0; //     default to 25 deg C, non-isothermal when ntemp == 8

    if(ncomp != ncompchombo)
      {
    	printf("ncomp in crunchflow is not equal to ncomp in Chombo\n");
    	exit(1);
      }
    else if(nspec != nspecchombo)
      {
    	printf("nspec in crunchflow is not equal to nspec in Chombo\n");
    	exit(1);
      }
    else if(nkin != nkinchombo)
      {
    	printf("nkin in crunchflow is not equal to nkin in Chombo\n");
    	exit(1);
      }

    ndim1 = nspec + nkin * MAX_PATH;

    // // modules/concentration.F:      REAL(DP), DIMENSION(:,:), ALLOCATABLE            :: as1
    // //
    
    // //    as1 = (double **) malloc((NBASIS + 1) * sizeof(double *));
    // //    for(size_t i = 0; i <= NBASIS; i++)
    // //      as1[i] = (double *) calloc(ndim1 + 1, sizeof(double));

    // //     ________________________ DEBYE HUCKEL _____________________
    // //

    for(int i = 0; i < TPOINTS; i++)
        if ( tempc[i] == -333.0 )
          break;
    	else
    	  ntemp++;

      if (ntemp == 0)
    	{
    	  //  backward compatibility - no temp data provided, assume 25 deg C
    	  ntemp = 1;
    	  tempc[0] = 25.0;
    	}
      else if (ntemp == 1 && tempc[0] != 25.0)
    	{
    	  printf("error: if only one temperature point, temperature must be = 25 deg C\n");
    	  exit;
    	}
      else if (ntemp == 8 )
        ;
      else
    	{
    	  printf("error: number of temp points in input_test.txt must be 1 or 8\n");
    	  exit;
    	}

      for(int i = 0; i < ntemp; i++)
	if(a_dh[i] == -333.0 || b_dh[i]  == -333.0 || b_dot[i] == -333.0)
	  {
	    printf("error: a_dh, b_dh or b_dot have not been initialized for one or more temp points\n");
	    exit;
	  }

      if(ntemp == 1)
	{
	  adh[1]  = a_dh[1];
	  bdh[1]  = b_dh[1];
	  bdot[1] = b_dot[1];
	}
      else // ntemp == 8
	{
	  for(int i = 0; i < ntemp; i++)
	    {
	      vec[i][1-1] = log(tempc[i] + TK);
	      vec[i][2-1] = 1.0;
	      vec[i][3-1] = tempc[i] + TK;
	      vec[i][4-1] = 1.0/(tempc[i] + TK);
	      vec[i][5-1] = 1.0/((tempc[i] + TK)*(tempc[i] + TK));

	      vecgam[i][1-1] = 1.0;
	      vecgam[i][2-1] = tempc[i];
	      vecgam[i][3-1] = pow(tempc[i],2);
	      vecgam[i][4-1] = pow(tempc[i],3);
	      vecgam[i][5-1] = pow(tempc[i],4);
	    }

	  //       fit gamma
	  status = fitgamma_cpp(adh);
	  for(int i = 0; i < NBASIS; i++)
	    adhcoeff[i] = bvec[i];
	  status = fitgamma_cpp(bdh);
	  for(int i = 0; i < NBASIS; i++)
	    bdhcoeff[i] = bvec[i];
	  status = fitgamma_cpp(bdot);
	  for(int i = 0; i < NBASIS; i++)
	    bdtcoeff[i] = bvec[i];
	}

      neqn = ncomp;

      //     _______________________ ALLOCATE _________________________
      //


      //     _______________________ COMPONENTS _______________________
      //
      //     read all components   
      if (ncomp <= 0)
	{
	  printf("no components\n");
	  exit;
	}
      else
	{
	  for(size_t i = 0; i < ncomp; i++)
	    {
	      ulab[i] = (char *)malloc((strlen(components[i].name) + 1) * sizeof(char));
	      ulab[i] = strcpy(ulab[i], components[i].name);
	      chg[i]  = components[i].charge;
	      // printf("initialize_cpp: component %d, charge %f, name %s\n",i,chg[i],components[i].name);
	      wtaq[i] = components[i].molecular_weight;
	      acmp[i] = components[i].a_zero;
	    }
	}

      //     ______________________ COMPLEXATION _____________________
      if (nspec < 0)
	{
	  printf("negative aqueous reactions");
	  exit;
	 }
      else if (nspec == 0)
         printf("no aqueous reactions\n");
      else
	{
	  //     loop over reactions
	  for(int i = 0; i < nspec; i++) // i is column index in muaq
	    {
	      // initialize row j in muaq[ncomp][nspec]
	      for(int j = 0; j < ncomp; j++)
		muaq[j][i] = 0.0;

	      // example stoichiometry: -2.0 'H+'  1.0 'CO2(aq)'  1.0 'Ca++'
	      char *tokenPtr = strtok(complexes[i].stoichiometry, " "); // begin tokenizing stoichiometry
	      int tokenNum = 0;
	      double mu = 0.0;
	      char *p;
	      char *q;

	      // continue tokenizing line until tokenPtr becomes NULL
	      while (tokenPtr != NULL) 
		{
		  if(tokenNum % 2 == 0)
		    {
		      // printf("stoichiometry: mu %s\n", tokenPtr);
		      sscanf(tokenPtr,"%lf",&mu);
		    }
		  else
		    {
		      // printf("stoichiometry: component %s\n", tokenPtr);
		      if((p = strpbrk(tokenPtr,"'")))
			{
			  q = strrchr(p,'\'');
			  if(q)
			    *q = '\0';
			  int j;
			  for(j = 0; j < ncomp; j++)
			    if(!strcmp(p+1, ulab[j]))
			      {
				muaq[j][i] = mu;
				break;
			      }
			  if(j >= ncomp)
			    {
			      fprintf(stderr,"could not find component %s for reaction %s\n",p,complexes[i].name);
			      exit(1);
			    }
			}
		      mu = 0.0;
		    }
		  tokenNum++;
		  tokenPtr = strtok(NULL, " "); // get next token
		}

	      chg[ncomp+i] = complexes[i].charge;
	      wtaq[ncomp+i] = complexes[i].molecular_weight;
	      acmp[ncomp+i] = complexes[i].a_zero;
	      namcx[i] = (char *)malloc((strlen(complexes[i].name) + 1) * sizeof(char));
	      namcx[i] = strcpy(namcx[i], complexes[i].name);

	      // fprintf(stderr,"initialize_cpp: species %s (%d) charge: %f, keq: %f, %d %f\n",
	      // 	      namcx[i],ncomp+i,chg[ncomp+i],complexes[i].keq,ntemp,temp);

	      // equilibrium constant stuff
	      if(ntemp > 1)
		{
		  fit_(&nbasis,&ntemp,complexes[i].keq,bvec,&vec[0][0],&iflgint,INT,&ntt,namcx[i]);

		  if(iflgint == 1)
		    {
		      nncnt = 0;
		      for(int j = 0; j < ntemp; ++j)
			if (INT[j] == 1)
			  {
			    nncnt = nncnt + 1;
			    temptmp[nncnt-1] = tempc[j];
			  }

		      if (ntt == 1)
			if (temp != temptmp[0])
			  {
			    printf(" Only one logK in database at T(C): %f\n", temptmp[0]);
			    printf(" Temperature of condition:         %f\n",temp);
			    printf(" Species missing log Ks: %s\n", namcx[i]);
			    printf(" Exiting due to input file or parsing error.\n");
			    exit(-1);
			  }
		    }

		  // put it in the permanent vector of coefficients
		  for(int j = 0; j < NBASIS; ++j)
		    // double as1[NBASIS][12]; // transposed from fortran as1(12,5)
		    as1[j][i] = bvec[j];
		}
	      else if (ntemp == 1)
		eqhom[i] = complexes[i].keq[0];
	    } // for(int i = 0; i < nspec; i++)
	} // COMPLEXATION

      //     ______________________ MINERALS _________________________    
      
      //     pre-dimension
      np = MAX_PATH; // global, needed by reaction_cpp_
      int max_ip;
      int ip;
      int id; // integer(i4b) :: id,nd
      double time_scale;

      //     read all minerals
      if (nkin < 0)
	{
	  printf("negative minerals\n");
	  exit(-1);
	}
      else if (nkin == 0)
	printf("no minerals\n");
      else 
	{
	  //     (pre-)allocate variables
	  np = MAX_PATH; // max number of paths for all mineral reactions
	  max_ip = 0;
	  nd = ncomp+nspec; // max number of dependencies

	  for(size_t i = 0; i < nkin; ++i)
	    for(size_t j = 0; j < np; ++j)
	      AffinityDepend1[i][j] = 1.0; // initialize AffinityDepend1

	  int nptot = 0;

	  // loop over reactions
	  for(int nkinIndex = 0; nkinIndex < nkin; nkinIndex++)
	    {
	      // initialize mumin
	      for(int ncompIndex = 0; ncompIndex < ncomp; ++ncompIndex)
		for(int npIndex = 0; npIndex < np; ++npIndex)
		  mumin[ncompIndex][nkinIndex][npIndex] = 0.0; // transpose of (np,nkin,ncomp), mumin(:,i,:) = 0.0d0

	      // example mineral reaction stoichiometry: 1.0 'Ca++'  -2.0 'H+'  1.0 'CO2(aq)'
	      char *tokenPtr = strtok(minerals[nkinIndex].stoichiometry, " "); // begin tokenizing stoichiometry
	      int tokenNum = 0;
	      char *p;
	      char *q;
	      double mu;

	      // continue tokenizing line until tokenPtr becomes NULL
	      while (tokenPtr != NULL) 
		{
		  if(tokenNum % 2 == 0)
		    {
		      // printf("mineral stoichiometry: mu %s\n", tokenPtr);
		      sscanf(tokenPtr,"%lf",&mu);
		    }
		  else
		    {
		      // printf("mineral stoichiometry: component %s\n", tokenPtr);
		      if((p = strpbrk(tokenPtr,"'")))
			{
			  q = strrchr(p,'\'');
			  if(q)
			    *q = '\0';
			  if(!strcmp(p+1, "default"))
			    {
			      break;
			    }
			  else 
			    {
			      int j;
			      for(j = 0; j < ncomp; ++j)
				if(!strcmp(p+1, ulab[j]))
				  {
				    for(int k = 0; k < np; ++k)
				      {
					mumin[j][nkinIndex][k] = mu;
					// fprintf(stderr,"mumin[%d][%d][%d] = %.4e\n",j,nkinIndex,k,mu);
				      }
				    break;
				  }
			      if (j == ncomp)
				{
				  printf("mumin: could not find component %s for mineral reaction %s\n",
					 p+1, minerals[nkinIndex].name);
				  exit(-1);
				}
			    }
			}
		    }
		  tokenNum++;
		  tokenPtr = strtok(NULL, " "); // get next token
		}

	      namin[nkinIndex] = minerals[nkinIndex].name;
	      ip = 0;

	      // volume of minerals
	      volmol[nkinIndex] = minerals[nkinIndex].molar_volume * 1.0e-6;

	      // equilibrium constant stuff
	      if (ntemp > 1)
		{
		  //   void fit_(int *nbasis, int *ntemp, double *alogk0, double *bvecF, double *vecF, int *iflgint, int *inoutint, int *ntt, char *nameTransfer)
		  // fprintf(stderr,"fit: cpp %s, %d\n",namin[nkinIndex],nkinIndex);
		  fit_(&nbasis, &ntemp, minerals[nkinIndex].keq, bvec, &vec[0][0], &iflgint, INT, &ntt, namin[nkinIndex]);

		  if (iflgint == 1)
		    {
		      nncnt = 0;
		      for(int j = 0; j < ntemp; ++j)
			{
			  if (INT[j] == 1)
			    {
			      nncnt = nncnt + 1;
			      temptmp[nncnt] = tempc[nkinIndex];
			   }
			}

		      if (ntt == 1)
			{
			  if (temp != temptmp[0])
			    {
			      printf("Only one logK in database at T(C): %f\n",temptmp[0]);
			      printf(" Temperature of condition:          %f\n",temp);
			      printf(" Species missing log Ks: %s\n", namin[nkinIndex]);
			      printf("\n");
			      exit(-1);
			    }
			}
		    } // if (iflgint == 1)
		} // if (ntemp > 1)

	      // end of equilibrium constant stuff

	      // read pathways
	      if (read_kinetics)
		{

		  for(int ipath=0; ipath < MAX_PATH; ++ipath)
		    {
		      // fprintf(stderr,"pathway %d dependency list: ", ipath);
		      // printSpecies(pathways[ipath].dependencyList);

		      if (pathways[ipath].name == NULL)
			{
			  continue;
			}
		      else if (strcmp(pathways[ipath].name, namin[nkinIndex]) != 0)
			{
			  continue;
			}
		      else
			{
			  id = 0;
			  // ip = ip + 1;

			  Species *dependency = pathways[ipath].dependencyList; // head node 

			  for(int k = 0; k < ncomp + nspec; ++k)
			    {
			      // kth dependency in linked list
			      //
			      if(dependency == NULL)
				break; // end of list
			      else if (!strcmp(dependency->name, "default"))
				break;  // tail 
			      else if (!strncmp(dependency->name, "tot_", 4))
				{
				  //   double itot_min[1][MAX_PATH][15]; //     ! allocate(itot_min(nd,np,nkin),stat=ierr); itot_min = 0   

				  itot_min[nkinIndex][ip][id] = 1; // itot_min(id+1,ip,i) = 1
				  dependency->name = trimwhitespace(dependency->name);
				  dependency->name = strchr(dependency->name, '_') + 1;
				}

			      int j = 0;
			      for(j = 0; j < ncomp + nspec; ++j)
				{
				  if (j < ncomp)
				    {
				      if(!strcmp(dependency->name, ulab[j]))
					{
					  // id = id + 1;
					  idepend[nkinIndex][ip][id] = j + 1; // idepend(id,ip,i) = j;
					  depend[nkinIndex][ip][id] = dependency->nu; // depend(id,ip,i)= dependence(k)%coefficient
					  id = id + 1;
					  break;
					}
				    }
				  else
				    {
				      // j >= ncomp
				      if(!strcmp(dependency->name, namcx[j - ncomp]))
					{
					  // id = id + 1;
					  idepend[nkinIndex][ip][id] = j + 1; // idepend(id,ip,i) = j;
					  depend[nkinIndex][ip][id] = dependency->nu; // depend(id,ip,i)= dependence(k)%coefficient
					  id = id + 1;
					  break;
					}
				    }
				} // for(int j = 0; j < ncomp + nspec; ++j)
			      
			      if (j >= ncomp + nspec)
				{
				  fprintf(stderr,"could not find species %s in pathway %s for mineral reaction %s\n",
					  dependency->name, 
					  pathways[ipath].label, 
					  pathways[ipath].name);
				  exit(1);
				}
			      if(dependency->next != NULL)
				dependency = dependency->next;
			      else
				break; // no more dependent species
			      
			    } // for(int k = 0; k < ncomp + nspec; ++k)

			  // crunchflow internal time units are years
			  time_scale = 365.0*24.0*60.0*60.0;
			  rate0[nkinIndex][ip] = pow(10,pathways[ipath].rate)*time_scale; // rate0(ip,i) = (10**rate)*time_scale ! corrected bug 2012-10-04
			  ndepend[nkinIndex][ip] = id; // ndepend(ip,i) = id
			  AffinityDepend1[nkinIndex][ip] = pathways[ipath].m1; // AffinityDepend1(ip,i) = m1 ! 2011-11-30

			  // changes because of temperature dependence changes, 
			  // index for alnk and as1 includes pathways

                          if (ntemp > 1)
			    {
			      for(int j = 0; j < nbasis; ++j)
				  as1[j][nptot + nspec] = bvec[j];
			      // fprintf(stderr,"nptot = %d\n",nptot);
			    }
			  else if (ntemp == 1)
			     {
			       alnk[nptot - 1] = - minerals[nkinIndex].keq[0]; // backward compatible 
			       // (sign was incorrectly changed in first CrunchFlowLite version)
			       // (correcting it now will result in problems)
			     }
			  
			  nptot = nptot + 1;
	     
			} // name == namin(i) pathway for this mineral, if (pathways[ipath].name == NULL)
      
		      // breaks,  free(): invalid pointer: 0x00000000018a9670 * BUG 1
		      ip = ip + 1;
      
		    } // for(int ipath=0; ipath < MAX_PATH; ++ipath)

                  nreactmin[nkinIndex] = ip;
                  max_ip = max(ip,max_ip);
		} // if (read_kinetics), end read kinetic pathways
	    } // for(int nkinIndex = 0; nkinIndex < nkin; nkinIndex++), loop over reactions
	} // read all minerals

      // dumpMuMin(np, nkin, ncomp, nspec, mumin, ulab);

      // figure out max dimensions
      nd = maxval(nkin, np, ndepend);
      np = max_ip;
      
      // reallocate things
      ndim1 = np;
      ndim2 = nkin;
      ndim3 = ncomp;

      // fprintf(stderr,
      // 	      "initialize.cpp: reallocate mumin to dimension [%d][%d][%d]\n",
      // 	      ndim1,ndim2,ndim3);

      // printarray("mumin", 6, 1, 10, mumin); // transpose of (np,nkin,ncomp)

      // stoichchombo(1,1:nkin,1:ncomp) = mumin(1,1:nkin,1:ncomp)
      for(int i = 0; i < ncomp; ++i)
	for(int j = 0; j < nkin; ++j)
	  for(int k = 0; k < np; ++k)
	    stoichchombo[i][j][k] = mumin[i][j][k];
  
      // printarray("stoichchombo", 6, 1, 10, mumin);
      // printarray("depend", nkinchombo, MAX_PATH, MAX_DEPEND, depend);
      // printintarray("idepend", nkinchombo, MAX_PATH, MAX_DEPEND, idepend);

      // for(int i = 0; i < ncomp; ++i)
      // 	for(int j = 0; j < IKIN; ++j)
      // 	  {
      // 	    mukin[i][j] = 0.0;
      // 	    rdkin[i][j] = 0.0;
      // 	  }

      // encapsulated into os3d_newton
      // for(int k = 0; k < nz; ++k)
      // 	for(int j = 0; j < ny; ++j)
      // 	  for(int i = 0; i < nx; ++i)
      // 	    for(int l = 0; l < IKIN; ++l)
      // 	      raq_tot[k][j][i][l] = 0.0;
      
      // for(int k = 0; k < nz; ++k)
      // 	for(int j = 0; j < ny; ++j)
      // 	  for(int i = 0; i < nx; ++i)
      // 	    for(int l = 0; l < nkin; ++l)
      // 	      {
      // 		dppt[k][j][i][l] = 0.0; allocate(dppt(nkin,nx,ny,nz),stat=ierr); dppt = 0.d0    
      // 		u_rate[k][j][i][l] = 0.0; 
      //                area[k][j][i][l] = 100.0;
      // 		volfx[k][j][i][l] = 1.0; encapsulated in crunchflowloop_cpp
      // 		if (l == 1) volfx[k][j][i][l] = 0.0; if (nkin==2) volfx(2,:,:,:) = 0.0d0
      // 	      }
      
      // ncomp
      //
      // for(int j = 0; j < ncomp; ++j)
      // 	{
      // 	  jac_sat[j] = 0.0;
      // 	}

      // nkin
      //
      // for(int j = 0; j < nkin; ++j)
      // 	{
      // 	  surf[j] = 0.0;
      // 	}
      
      // np x ncomp
      //
      // for(int i = 0; i < MAX_PATH; ++i)
      // 	for(int j = 0; j < ncomp; ++j)
      // 	  {
      // 	    jac_pre[i][j] = 0.0;
      // 	  }

      // nkin x np
      //
      // for(int i = 0; i < nkin; ++i)
      // 	for(int j = 0; j < MAX_PATH; ++j)
      // 	  {
      // 	    actenergy[i][j] = silog[i][j] = siln[i][j] = si[i][j] = rmin[i][j] = pre_rmin[i][j] = snorm[i][j] = 0.0;
      // 	  }

      // nkin x ncomp
      //
      // for(int i = 0; i < nkin; ++i)
      // 	for(int j = 0; j < ncomp; ++j)
      // 	  {
      // 	    decay_correct[i][j] = 1.0;
      // 	  }

      // 1D: ncomp+nspec, encapsulated in crunchflowloop
      //
      // for(int i = 0; i < ncomp + nspec; ++i)
      // 	sppTMP[i] = 0.0;

      // 1D: neqn
      //
      // for(int i = 0; i < neqn; ++i)
      // 	fxx[i] = fxmax[i] = 0.0;

      // nkin x np x ncomp
      //
      // for(int i = 0; i < nkin; ++i)
      // 	for(int j = 0; j < MAX_PATH; ++j)
      // 	  for(int k = 0; k < ncomp; ++k)
      // 	  {
      // 	    jac_rmin[i][j][k] = 0.0;
      // 	  }

      // neqn,nx,ny,nz
      //
      for(int k = 0; k < nz; ++k)
	for(int j = 0; j < ny; ++j)
	  for(int i = 0; i < nx; ++i)
	    for(int l = 0; l < neqn; ++l)
	      {
		s[k][j][i][l] = 0.0; // allocate(s(neqn,nx,ny,nz),stat=ierr); s = 0.0d0
		sn[k][j][i][l] = 0.0; // allocate(sn(neqn,nx,ny,nz),stat=ierr); sn = 0.0d0
	      }

      // neqn,neqn,nx,ny,nz, encapsulate in crunchflowloop
      //
      // for(int k = 0; k < nz; ++k)
      // 	for(int j = 0; j < ny; ++j)
      // 	  for(int i = 0; i < nx; ++i)
      // 	    for(int l = 0; l < neqn; ++l)
      // 	      for(int m = 0; m < neqn; ++m)
      // 		fjac[k][j][i][l][m] = 0.0;

      // neqn,neqn, encapsulate in os3d_newton, zeroed in jac_local()
      //
      // for(int l = 0; l < neqn; ++l)
      // 	for(int m = 0; m < neqn; ++m)
      // 	  fjac_loc[l][m] = 0.0; // allocate(fjac_loc(neqn,neqn),stat=ierr); fjac_loc = 0.0d0

      // neqn
      //
      // for(int l = 0; l < neqn; ++l)
      // 	sumjackin[l] = sbdry[l] = 0.0; // sumjackin encapsulated in assemble_local, sbdry not actually used
      
      // nx,ny,nz
      //
      for(int k = 0; k < nz; ++k)
	for(int j = 0; j < ny; ++j)
	  for(int i = 0; i < nx; ++i)
	    {
	      sion[k][j][i] = 0.0;
	      t[k][j][i] = 25.0;
	    }


      // read all solutions   
      for(unsigned int i=0; i < MAX_SOL; ++i)
	{
	  if(solutions[i].name == NULL)
	    break; // no more solutions

	  Species *species = solutions[i].concentrationList; // head node 

	  for(int k = 0; k < ncomp + nspec; ++k)
	    {
	      // kth dependency in linked list
	      //
	      if(species == NULL)
		break; // end of list
	      else if (!strcmp(species->name, "default"))
		break;  // tail 
	      int j;
	      for(j = 0; j < ncomp + nspec; ++j)
		{
                  if (j < ncomp)
		    {
		      if (!strcmp(species->name, ulab[j]))
			{
			  sp10[0][0][0][j] = species->nu;
			  // fprintf(stderr,"sp10[0][0][0][%d] = %.3e (%s)\n",j,species->nu,ulab[j]);
			  break;
			}
		    }
                  else if (j >= ncomp) 
		    {   
		      if (!strcmp(species->name, namcx[j-ncomp]))
			{
			  sp10[0][0][0][j] = species->nu;
			  // fprintf(stderr,"sp10[0][0][0][%d] = %.3e (%s)\n",j,species->nu,namcx[j-ncomp]);
			  break;
			}
		    }
		}

               if (j >= ncomp+nspec)
		 {
		   fprintf(stderr,"could not find species %s in solution %s\n",
			   species->name, solutions[i].name);
		   exit(1);
		 }
	      
	      if(species->next != NULL)
		species = species->next;
	      else
		break; // no more species in list
	    }

	  t[0][0][0] = temp;

	  if (!strcmp(solutions[i].name,"inlet"))
	    {
	      // this is the inlet boundary

	      cfgamma(ncomp,
		      nspec,
		      0,
		      0,
		      0,
		      nx,
		      ny,
		      sp10,
		      t,
		      adhcoeff,
		      bdhcoeff,
		      bdtcoeff,
		      sion,
		      ulab,
		      acmp,
		      chg,
		      gam);

	      // int jx = 0; 
	      // int jy = 0; 
	      // int jz = 0;


		     totconc_cpp_(ncomp, 
				  nspec, 
				  neqn,
				  1, 
				  1, 
				  1, 
				  nx, 
				  ny, 
				  nz, 
				  muaq,
				  sp10,
				  s);


	      // totconc_c(ncomp, 
	      // 	       nspec, 
	      // 	       jx, 
	      // 	       jy, 
	      // 	       jz,
	      // 	       muaq, 
	      // 	       sp10, 
	      // 	       sp, 
	      // 	       s, 
	      // 	       *nx, 
	      // 	       *ny, 
	      // 	       *nz, 
	      // 	       neqn);

	      // store it in sbdry, not actually used, disabled
	      // for(size_t i = 0; i < neqn; ++i)
	      // 	sbdry[i] = s[0][0][0][i];

	      for(size_t i = 0; i < ncomp; ++i)
		{
		  inflowchombo[i] = s[0][0][0][i]/1000.0;
		  inflowchombosp10[i] = sp10[0][0][0][i]/1000.0;
		}


      for(int k = 0; k < nz; ++k)
	for(int j = 0; j < ny; ++j)
	  for(int i = 0; i < nx; ++i)
	    for(int l = 0; l < neqn; ++l)
	      s[k][j][i][l] = 0.0; // allocate(s(neqn,nx,ny,nz),stat=ierr); s = 0.0d0

      for(int k = 0; k < nz; ++k)
	for(int j = 0; j < ny; ++j)
	  for(int i = 0; i < nx; ++i)
	    for(int l = 0; l < ncomp + nspec; ++l)
	      {
		sp10[k][j][i][l] = 0.0; // allocate(sp10(ncomp+nspec,nx,ny,nz),stat=ierr); sp10 = 0.0d0
		gam[k][j][i][l] = 0.0; // allocate(gam(ncomp+nspec,nx,ny,nz),stat=ierr); gam = 0.0d0
	      }

	    }
	  else if (!strcmp(solutions[i].name,"inlet2"))
	    {
	      // this is a second the inlet boundary

	      cfgamma(ncomp,
		      nspec,
		      0,
		      0,
		      0,
		      nx,
		      ny,
		      sp10,
		      t,
		      adhcoeff,
		      bdhcoeff,
		      bdtcoeff,
		      sion,
		      ulab,
		      acmp,
		      chg,
		      gam);

	      // int jx = 0; 
	      // int jy = 0; 
	      // int jz = 0;

		     totconc_cpp_(ncomp, 
				  nspec, 
				  neqn,
				  1, 
				  1, 
				  1, 
				  nx, 
				  ny, 
				  nz, 
				  muaq,
				  sp10,
				  s);


	      // totconc_c(ncomp, 
	      // 	       nspec, 
	      // 	       jx, 
	      // 	       jy, 
	      // 	       jz,
	      // 	       muaq, 
	      // 	       sp10, 
	      // 	       sp, 
	      // 	       s, 
	      // 	       *nx, 
	      // 	       *ny, 
	      // 	       *nz, 
	      // 	       neqn);

	      // store it in sbdry          
	      // for(size_t i = 0; i < neqn; ++i)
	      // 	sbdry[i] = s[0][0][0][i];

	      for(int k = 0; k < nz; ++k)
		for(int j = 0; j < ny; ++j)
		  for(int i = 0; i < nx; ++i)
		    for(int l = 0; l < neqn; ++l)
		      s[k][j][i][l] = 0.0; // allocate(s(neqn,nx,ny,nz),stat=ierr); s = 0.0d0

	      for(int k = 0; k < nz; ++k)
		for(int j = 0; j < ny; ++j)
		  for(int i = 0; i < nx; ++i)
		    for(int l = 0; l < ncomp + nspec; ++l)
		      {
			sp10[k][j][i][l] = 0.0; // allocate(sp10(ncomp+nspec,nx,ny,nz),stat=ierr); sp10 = 0.0d0
			gam[k][j][i][l] = 0.0; // allocate(gam(ncomp+nspec,nx,ny,nz),stat=ierr); gam = 0.0d0
		      }
	      
	    }
	  else if (!strcmp(solutions[i].name,"initial"))
	    {
	      //  this is the initial condition in the domain

	      cfgamma(ncomp,
		      nspec,
		      0,
		      0,
		      0,
		      nx,
		      ny,
		      sp10,
		      t,
		      adhcoeff,
		      bdhcoeff,
		      bdtcoeff,
		      sion,
		      ulab,
		      acmp,
		      chg,
		      gam);

	      // int jx = 0; 
	      // int jy = 0; 
	      // int jz = 0;


		     totconc_cpp_(ncomp, 
				  nspec, 
				  neqn,
				  1, 
				  1, 
				  1, 
				  nx, 
				  ny, 
				  nz, 
				  muaq,
				  sp10,
				  s);


	      // totconc_c(ncomp, 
	      // 	       nspec, 
	      // 	       jx, 
	      // 	       jy, 
	      // 	       jz,
	      // 	       muaq, 
	      // 	       sp10, 
	      // 	       sp, 
	      // 	       s, 
	      // 	       *nx, 
	      // 	       *ny, 
	      // 	       *nz, 
	      // 	       neqn);

	      for(size_t i = 0; i < ncomp; ++i)
		{
		  initchombo[i] = s[0][0][0][i]/1000.0;
		}

	      for(size_t i = 0; i < ncomp + nspec; ++i)
		{
		  initchombosp10[i] = sp10[0][0][0][i];
		  initchombosp[i] = log(sp10[0][0][0][i]);
		}

	      for(size_t i = 0; i < ncompchombo + nspecchombo; ++i)
		initgamma[i] = gam[0][0][0][i];

	      // reset to zero
	      //
	      for(size_t i = 0; i < neqn; ++i)
		s[0][0][0][i] = 0.0; // allocate(s(neqn,nx,ny,nz),stat=ierr); s = 0.0d0

	      for(size_t i = 0; i < ncompchombo + nspecchombo; ++i)
		{
		  sp10[0][0][0][i] = 0.0; // allocate(sp10(ncomp+nspec,nx,ny,nz),stat=ierr); sp10 = 0.0d0
		  gam[0][0][0][i] = 0.0; // allocate(gam(ncomp+nspec,nx,ny,nz),stat=ierr); gam = 0.0d0
		}
	    }
	  else 
	    {     
	      fprintf(stderr,"name of condition %s not provided or recognized\n",
		      solutions[i].name);
		 exit(1);
	    }
	} // read all solutions

      // retrieve initial concentrations from 2,1,1 to entire domain
      for(size_t k = 0; k < nz; ++k)
	for(size_t j = 0; j < ny; ++j)
	  for(size_t i = 0; i < nx; ++i)
	    {
	      for(size_t l = 0; l < neqn; ++l)
		s[k][j][i][l] = initchombo[l] * 1000.0; // s(1:neqn,2,1,1), allocate(s(neqn,nx,ny,nz),stat=ierr); s = 0.0d0
	      for(size_t l = 0; l < ncompchombo + nspecchombo; ++l)
		{
		  sp10[k][j][i][l] = initchombosp10[l]; // sp10(1:ncomp+nspec,2,1,1)
		  gam[k][j][i][l]  = initgamma[l]; // gam(1:ncomp+nspec,2,1,1)
		}
	    }

      // initialize sn, and sp
      for(int k = 0; k < nz; ++k)
	for(int j = 0; j < ny; ++j)
	  for(int i = 0; i < nx; ++i)
	    {
	      for(int l = 0; l < neqn; ++l)
		sn[k][j][i][l] = s[k][j][i][l];
	      for(int l = 0; l < ncompchombo + nspecchombo; ++l)
		sp[k][j][i][l] = log(sp10[k][j][i][l]);
	    }

      // ______________________ HARDWIRED ________________________

      // for(size_t k = 0; k < nz; ++k)
      // 	for(size_t j = 0; j < ny; ++j)
      // 	  for(size_t i = 0; i < nx; ++i)
      // 	    {
      // 	      // por[k][j][i] = 1.0; //hardwired (porosity), encapsulated in crunchflowloop
      // 	      // satliq[k][j][i] = 1.0; //hardwired (saturation), encapsulated in crunchflowloop
      // 	      // ro[k][j][i] = 1000.0; //hardwired (density)
      // 	      //	      H2Oreacted[k][j][i] = 1.0;
      // 	    }

      // for(size_t k = 0; k < nz + 2; ++k)
      // 	for(size_t j = 0; j < ny + 2; ++j)
      // 	  for(size_t i = 0; i < nx + 3; ++i)
      // 	    {
      // 	      xgram[k][j][i] = 1.0;
      // 	      xgramOld[k][j][i] = 1.0;
      // 	    }

      // for(size_t i = 0; i < ncompchombo; ++i)
      // 	distrib[i] = 0.0;

      // for(size_t i = 0; i < neqn; ++i)
      // 	indd[i] = 0.0;

    return(0);
  }

  void printintarray(const char *name, int m, int n, int l, int a[][n][l])
  {
    int i, j, k;
    for (i = 0; i < m; i++)
      for (j = 0; j < n; j++)
	for (k = 0; k < l; k++)
	  fprintf(stderr,"printarray %s[%d][%d][%d] = %d\n",
		  name, i, j, k, a[i][j][k]);
  }

  void printarray(const char *name, int m, int n, int l, double a[][n][l])
  {
    int i, j, k;
    for (i = 0; i < m; i++)
      for (j = 0; j < n; j++)
	for (k = 0; k < l; k++)
	  fprintf(stderr,"printarray %s[%d][%d][%d] = %.2e\n",
		  name, i, j, k, a[i][j][k]);
  }

  // Determines the maximum value of the elements in an array
  //
  int maxval(int m, int n, int a[][n])
  {
    int i, j;
    int max = a[0][0];
    for (i = 0; i < m; i++)
      for (j = 0; j < n; j++)
	if (a[i][j] > max)
	  max=a[i][j];
    return(max);
  }

  /*
  int realloc_as1(int *ndim1, int *ndim2)
  {
    fprintf(stderr,"Invoking realloc_as1 (ndim1=%d,ndim2=%d)...\n", *ndim1, *ndim2);
    if(*ndim1 != 0 && *ndim2 != 0)
      {
	as1 = (double **) malloc((*ndim2 + 1)* sizeof(double *));
	for(size_t i = 0; i <= *ndim2; i++)
	  as1[i] = (double *) calloc(*ndim1 + 1, sizeof(double));
      }
    return(0);
  }

  int update_as1(int *i)
  {
    fprintf(stderr,"Invoking update_as1 (i=%d)...\n", *i);
    for(size_t j = 1; j <= NBASIS; j++)
      as1[j][*i] = bvec[j];
    return(0);
  }
  */

  char *trimwhitespace(char *str)
  {
    char *end;

    // Trim leading space
    while(isspace((unsigned char)*str)) str++;

    if(*str == 0)  // All spaces?
      return str;

    // Trim trailing space
    end = str + strlen(str) - 1;
    while(end > str && isspace((unsigned char)*end)) end--;

    // Write new null terminator character
    end[1] = '\0';

    return str;
  }

  void dumpMuMin(const int np, 
		 const int nkin, 
		 const int ncomp, 
		 const int nspec,
		 const double mumin[ncomp][nkin][MAX_PATH],
		 char *ulab[ncomp + nspec])
  {
    // double mumin[6][1][MAX_PATH]; // transpose of (np,nkin,ncomp)
    // 
    // if (stoichiometry(k)%name == ulab(j)) then
    //  mumin(:,i,j) = stoichiometry(k)%mu
    //
    // i is reaction i=1,nkin
    // j is component j=1,ncomp ulab[j]
    //
    fprintf(stderr,"mumin table (rows are reactions, columns are components)\n");
    fprintf(stderr,"--------------------------------------------------------\n");
    for(size_t j = 0; j < ncomp; ++j)
      fprintf(stderr,"%8s",ulab[j]);
    fprintf(stderr,"\n");

    for(size_t i = 0; i < nkin; ++i)
      {
	for(size_t j = 0; j < ncomp; ++j)
	    fprintf(stderr,"%8.2f",mumin[j][i][0]);
	fprintf(stderr,"\n");
      }
    fprintf(stderr,"\n");
  }

#ifdef __cplusplus
}
#endif
